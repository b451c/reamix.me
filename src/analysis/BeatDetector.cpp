...
#if REAMIX_HAS_ONNX
BeatDetector::BeatDetector(Ort::Session& externalSession)
...
bool BeatDetector::loadModel(const std::string& modelPath)
{
    ...
    try
    {
        env_ = std::make_unique<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "reamix");
                                      ...
                                      std::function<void(const std::string&, float)> progressCb)
{
    ...
    InferenceProcessor inference(*sessionPtr_);
    try {
...
DetectionResult BeatDetector::detectFile(const std::string& filePath,
                                          std::function<void(const std::string&, float)> progressCb)
{
    // REAPER API decodes all formats REAPER can open (mp3, flac, ogg, opus, aac, wav, aiff, wma, ...)
