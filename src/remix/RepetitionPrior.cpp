                                                ...
                                                double threshold)
{
    ...
    for (int i : pre_db_set) {
        ...
        for (int j : db_set) {
            ...
            // Successor view: beat j stands in for beat i+1. Count recurrence
            // hits along the diagonal through (i+1, j) over the bar before
            // and the bar after the junction.
            int hits = 0;
            for (int k = -ts; k < ts; ++k) {
