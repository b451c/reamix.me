// Copy of references/reabeat-template/src/Postprocessor.cpp with class renamed
// to PostprocessorReabeat. Do not edit — re-copy and re-apply the rename if
// REABeat's reference changes.
#include "PostprocessorReabeat.h"
#include <algorithm>
#include <cmath>
#include <limits>

PostprocessorReabeat::PostprocessorReabeat(float fps) : fps_(fps) {}

std::vector<float> PostprocessorReabeat::maxPool1d(const std::vector<float>& input, int kernelSize)
{
...
std::vector<int> PostprocessorReabeat::detectPeaks(const std::vector<float>& logits)
{
...
std::vector<int> PostprocessorReabeat::deduplicatePeaks(const std::vector<int>& peaks, int width)
{
    if (peaks.empty())
        return {};
...
PostprocessResultReabeat PostprocessorReabeat::process(const std::vector<float>& beatLogits,
                                                       const std::vector<float>& downbeatLogits)
{
    PostprocessResultReabeat result;
